  ...
  function processCommand(cmd) {
    switch (cmd) {
      ...
      case 'events':
        ...
        appendTerminalLine('[NON-TECH]: Esport Gaming (Free Fire) | IPL Auction | Mystery Box');
        break;
...
// Auto-load custom seal logo from localStorage if available
document.addEventListener('DOMContentLoaded', () => {
  const sealLogoEl = document.getElementById('collegeSealLogo');
  const savedSeal = localStorage.getItem('mahendra_seal_b64');
  if (sealLogoEl && savedSeal) {
    sealLogoEl.src = savedSeal;
  }
});